#include <stdio.h>

int
main (int argc, char **argv)
{
  int x0;
  int x1;
  int x2;
  int x3;
  int x4;
  int x5;
  int x6;
  int x7;
  int x8;
  int x9;
  int x10;
  int x11;
//Possible solution to enter:
// x0 = 0
// x1 = 7
// x2 = -3
// x3 = -8
// x4 = 0
// x5 = -1
// x6 = -6
// x7 = -4
// x8 = 5
// x9 = -2
// x10 = -6
// x11 = 0
  if ((1 * x3 * x8 * x8) + (1 * x6 * x7 * x11) + (-1 * x5 * x9 * x11) +
      (-6 * x1 * x7) == -32)
    {
      printf ("I am here at depth 11");
      if ((4 * x1 * x2 * x8) + (15 * x2 * x7 * x10) + (-1 * x5 * x9 * x10) +
	  (1 * x2 * x10) == -1470)
	{
	  printf ("I am here at depth 10");
	  if ((2 * x3 * x6 * x7) + (-4 * x8 * x8 * x10) + (-5 * x2 * x10) ==
	      126)
	    {
	      printf ("I am here at depth 9");
	      if ((-14 * x0 * x1 * x5) + (11 * x4 * x4 * x8) + (-3 * x1) ==
		  -21)
		{
		  printf ("I am here at depth 8");
		  if ((1 * x0 * x2 * x9) + (-1 * x5 * x8 * x9) +
		      (-1 * x0 * x1 * x10) == -10)
		    {
		      printf ("I am here at depth 7");
		      if ((10 * x2 * x3 * x7) + (-1 * x2 * x6 * x7) +
			  (4 * x0 * x6 * x10) + (-1 * x0 * x0) +
			  (184 * x8 * x10) == -6408)
			{
			  printf ("I am here at depth 6");
			  if ((2 * x7 * x7 * x7) + (11 * x5 * x7 * x11) +
			      (1 * x2) + (15 * x11) == -131)
			    {
			      printf ("I am here at depth 5");
			      if ((6 * x0 * x6 * x6) + (-6 * x4 * x7 * x11) ==
				  0)
				{
				  printf ("I am here at depth 4");
				  if ((-2 * x0 * x2 * x5) +
				      (-3 * x1 * x5 * x6) +
				      (-6 * x1 * x1 * x8) +
				      (1 * x1 * x8 * x11) == -1596)
				    {
				      printf ("I am here at depth 3");
				      if ((2 * x1 * x7 * x8) +
					  (-1 * x1 * x4 * x9) +
					  (-3 * x3 * x5 * x11) +
					  (2 * x9 * x11) + (1 * x10) == -286)
					{
					  printf ("I am here at depth 2");
					  if ((-3 * x5 * x7 * x10) +
					      (-1 * x1 * x4) +
					      (-19 * x4 * x10) == 72)
					    {
					      printf ("I am here at depth 1");

					    }
					  else
					    {
					      printf
						("I am at the else of depth 1");
					    }
					}
				      else
					{
					  printf
					    ("I am at the else of depth 2");
					}
				    }
				  else
				    {
				      printf ("I am at the else of depth 3");
				    }
				}
			      else
				{
				  printf ("I am at the else of depth 4");
				}
			    }
			  else
			    {
			      printf ("I am at the else of depth 5");
			    }
			}
		      else
			{
			  printf ("I am at the else of depth 6");
			}
		    }
		  else
		    {
		      printf ("I am at the else of depth 7");
		    }
		}
	      else
		{
		  printf ("I am at the else of depth 8");
		}
	    }
	  else
	    {
	      printf ("I am at the else of depth 9");
	    }
	}
      else
	{
	  printf ("I am at the else of depth 10");
	}
    }
  else
    {
      printf ("I am at the else of depth 11");
    }
}
