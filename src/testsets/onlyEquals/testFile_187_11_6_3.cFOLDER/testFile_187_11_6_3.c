#include <stdio.h>

int
main (int argc, char **argv)
{
  int x0;
  int x1;
  int x2;
  int x3;
  int x4;
  int x5;
//Possible solution to enter:
// x0 = 0
// x1 = -3
// x2 = -8
// x3 = -4
// x4 = -2
// x5 = -9
  if ((1 * x0 * x0 * x2) + (2 * x1 * x2 * x3) + (1 * x1 * x1 * x4) +
      (-1 * x3 * x5 * x5) + (1 * x4) == 112)
    {
      printf ("I am here at depth 11");
      if ((-2 * x3 * x4 * x4) + (-1 * x0 * x0 * x5) + (-1 * x5 * x5) == -49)
	{
	  printf ("I am here at depth 10");
	  if ((2 * x1 * x1 * x2) == -144)
	    {
	      printf ("I am here at depth 9");
	      if ((1 * x0 * x1 * x3) + (-130 * x0 * x3 * x3) +
		  (1 * x1 * x4 * x4) + (1 * x2 * x2 * x5) +
		  (-21 * x0 * x4 * x5) + (3 * x4 * x4 * x5) == -696)
		{
		  printf ("I am here at depth 8");
		  if ((-10 * x1 * x2 * x2) + (69 * x1 * x3 * x3) +
		      (-3 * x1 * x1 * x4) + (-105 * x2 * x4 * x4) +
		      (-1 * x5 * x5 * x5) == 2751)
		    {
		      printf ("I am here at depth 7");
		      if ((2 * x3 * x4 * x4) + (1 * x0 * x3) + (3 * x4) ==
			  -38)
			{
			  printf ("I am here at depth 6");
			  if ((3 * x0 * x2 * x2) + (1 * x2 * x2 * x2) +
			      (11 * x4 * x4 * x4) + (-1 * x5) == -591)
			    {
			      printf ("I am here at depth 5");
			      if ((-2 * x2 * x3 * x5) + (1 * x3 * x3 * x5) ==
				  432)
				{
				  printf ("I am here at depth 4");
				  if ((8 * x2 * x2 * x4) +
				      (-1 * x2 * x4 * x5) + (-1 * x0 * x1) ==
				      -880)
				    {
				      printf ("I am here at depth 3");
				      if ((1 * x0 * x0 * x3) +
					  (-4 * x1 * x2 * x3) +
					  (2 * x1 * x4 * x4) == 360)
					{
					  printf ("I am here at depth 2");
					  if ((8 * x1 * x1 * x1) +
					      (1 * x0 * x2 * x5) +
					      (2 * x0 * x1) + (2 * x2 * x2) ==
					      -88)
					    {
					      printf ("I am here at depth 1");

					    }
					  else
					    {
					      printf
						("I am at the else of depth 1");
					    }
					}
				      else
					{
					  printf
					    ("I am at the else of depth 2");
					}
				    }
				  else
				    {
				      printf ("I am at the else of depth 3");
				    }
				}
			      else
				{
				  printf ("I am at the else of depth 4");
				}
			    }
			  else
			    {
			      printf ("I am at the else of depth 5");
			    }
			}
		      else
			{
			  printf ("I am at the else of depth 6");
			}
		    }
		  else
		    {
		      printf ("I am at the else of depth 7");
		    }
		}
	      else
		{
		  printf ("I am at the else of depth 8");
		}
	    }
	  else
	    {
	      printf ("I am at the else of depth 9");
	    }
	}
      else
	{
	  printf ("I am at the else of depth 10");
	}
    }
  else
    {
      printf ("I am at the else of depth 11");
    }
}
